# Fortun SRL — COPY.md

> Copy definitivo del sito bilingue. **IT = versione primaria · EN = specchio fedele (ramo `/en`).**
> Tutti i dati non verificati sono marcati `[DA CONFERMARE: ...]` — non sostituire senza conferma del cliente.
> Form contatti = **4 campi dati** (Nome · Azienda · Email · Messaggio) + **checkbox consenso GDPR** (la checkbox NON conta come 5° campo).
> Generato da workflow nexus (copy IT + specchio EN) con verifica adversariale — 26 giugno 2026.

---

# 🇮🇹 ITALIANO — versione primaria

## NAV

**Logo / Wordmark:** FORTUN

**Voci di navigazione (ancore interne):**
- Posizionamento → `#posizionamento`
- Hub operativi → `#hub`
- Prodotti → `#prodotti`
- Trading & supply → `#servizi`
- Come operiamo → `#come-operiamo`
- Contatti → `#contatti`

**Toggle lingua:** IT / EN (permanente, a destra nella nav)

**CTA nav:** Contatta il team → `#contatti`

---

## SEZIONE 1 — HERO

**Eyebrow:** Trading internazionale · Prodotti petroliferi raffinati

**Headline:**
Trading e supply di prodotti petroliferi raffinati per mercati in continua evoluzione.

**Sub (riga fattuale):**
Sede a Roma · operativi su Rotterdam, Houston e Fujairah · diesel, jet fuel, GPL, fuel oil, crude oil.

**CTA primario:** Contatta il team → `#contatti`

---

## SEZIONE 2 — POSIZIONAMENTO

`#posizionamento`

**Eyebrow:** Chi è Fortun

**Headline:**
Una società di trading italiana, con una rete di fornitori e controparti mondiale.

**Body:**
Fortun SRL è una società italiana con sede a Roma. Operiamo nel trading di prodotti petroliferi raffinati: acquistiamo, vendiamo e coordiniamo forniture tra produttori, raffinerie, distributori e operatori industriali. Non estraiamo e non raffiniamo: muoviamo prodotto già a specifica, dai mercati dove viene immesso a quelli dove serve. Il nostro lavoro è sapere dove c'è disponibilità, dove c'è domanda, e gestire tutto ciò che sta in mezzo.

**Numeri-ancora:**
- **2022** — anno di costituzione della società
- **5** prodotti raffinati trattati
- 3 hub operativi su tre fusi orari — ARA, US Gulf, Middle East

---

## SEZIONE 3 — HUB OPERATIVI

`#hub`

**Eyebrow:** Dove operiamo

**Headline:**
Tre hub, sui principali snodi del prodotto raffinato.

**Intro:**
La nostra operatività si appoggia ai tre nodi che concentrano gran parte degli scambi mondiali di prodotti raffinati. Ogni hub ci dà accesso a stoccaggio, controparti e logistica nella sua area.

**Hub 1 — Rotterdam**
- **Macro-area:** ARA (Amsterdam–Rotterdam–Anversa)
- **Ruolo:** Punto di riferimento europeo per lo stoccaggio e la distribuzione di prodotti raffinati. Da qui coordiniamo le forniture verso il Nord Europa e il bacino atlantico.

**Hub 2 — Houston**
- **Macro-area:** US Gulf (Golfo del Messico, USA)
- **Ruolo:** Accesso alla capacità di raffinazione e ai flussi di esportazione del Golfo statunitense. Hub per le operazioni sul mercato americano e transatlantico.

**Hub 3 — Fujairah**
- **Macro-area:** Middle East (Emirati Arabi Uniti)
- **Ruolo:** Snodo per il bunkeraggio e il transito tra Golfo Persico, Asia e Africa orientale. Da qui presidiamo i flussi verso est.

---

## SEZIONE 4 — PRODOTTI

`#prodotti`

**Eyebrow:** Cosa trattiamo

**Headline:**
Cinque prodotti raffinati, trattati a specifica.

**Intro:**
Lavoriamo su prodotti standardizzati e riconosciuti dal mercato internazionale. Ogni fornitura è definita per specifica, quantità e punto di consegna.

**Prodotto 1 — Diesel EN590**
Gasolio per autotrazione conforme allo standard europeo EN590. Carburante di riferimento per trasporto su strada e impieghi industriali.

**Prodotto 2 — Jet Fuel A1**
Cherosene per aviazione (Jet A1). Carburante per turbine destinato al trasporto aereo commerciale.

**Prodotto 3 — GPL (LPG)**
Gas di petrolio liquefatto. Impiegato per riscaldamento, autotrazione e usi industriali.

**Prodotto 4 — Fuel Oil**
Olio combustibile per uso industriale e marittimo, incluso il bunkeraggio navale.

**Prodotto 5 — Crude Oil**
Greggio destinato alle raffinerie come materia prima di processo.

---

## SEZIONE 5 — TRADING & SUPPLY

`#servizi`

**Eyebrow:** Cosa facciamo

**Headline:**
Dal reperimento alla consegna, gestiamo l'intera operazione commerciale.

**Intro:**
Il nostro ruolo non si esaurisce nel mettere in contatto due parti. Seguiamo la fornitura lungo tutta la catena, dalla ricerca del prodotto al coordinamento della consegna.

**Servizio 1 — Trading internazionale di commodities**
Acquistiamo e vendiamo prodotti petroliferi raffinati sui mercati internazionali, gestendo prezzo, controparti e termini contrattuali.

**Servizio 2 — Ricerca e approvvigionamento globale**
Individuiamo disponibilità di prodotto presso raffinerie, produttori e fornitori, valutando origine, specifica e affidabilità della controparte.

**Servizio 3 — Coordinamento delle forniture**
Gestiamo i tempi e i passaggi della fornitura tra origine e destinazione, allineando volumi, logistica e punti di consegna.

**Servizio 4 — Facilitazione commerciale**
Strutturiamo le operazioni tra le parti, occupandoci di documentazione, termini e condizioni della transazione.

**Servizio 5 — Operazioni sui mercati internazionali**
Operiamo sui mercati globali del prodotto raffinato, leggendo disponibilità, domanda e flussi tra le aree.

---

## SEZIONE 6 — COME OPERIAMO

`#come-operiamo`

**Eyebrow:** Il nostro metodo

**Headline:**
Come lavoriamo, e perché chi tratta con noi sa cosa aspettarsi.

**Intro:**
In un mercato dove l'affidabilità della controparte conta quanto il prezzo, il modo in cui operiamo è parte di ciò che offriamo.

**Pilastro 1 — Operatività affidabile**
Trattiamo solo forniture che possiamo seguire fino in fondo. Definiamo termini chiari prima di impegnarci e rispettiamo quanto concordato su specifica, volumi e tempi.

**Pilastro 2 — Presenza globale**
Con hub su Rotterdam, Houston e Fujairah copriamo le aree dove si concentra la maggior parte degli scambi di prodotto raffinato. La presenza su più fusi orari ci permette di operare in continuità.

**Pilastro 3 — Approccio compliance-oriented**
Lavoriamo nel rispetto delle normative che regolano il commercio internazionale di prodotti petroliferi. Verifichiamo le controparti e documentiamo le operazioni lungo l'intera catena.

**Pilastro 4 — Relazioni strategiche**
Il trading di commodity si regge sulla continuità dei rapporti. Costruiamo relazioni durature con fornitori e controparti, perché un canale affidabile vale più di una singola transazione.

---

## SEZIONE 7 — CONTATTI

`#contatti`

**Eyebrow:** Parliamo di forniture

**Headline:**
Hai una richiesta di fornitura o di acquisto? Scrivici.

**Microcopy (sopra al form):**
Indicaci prodotto, quantità e mercato di riferimento. Ti rispondiamo con i prossimi passi.

**FORM — campi:**

1. **Nome**
 - Label: Nome e cognome
 - Placeholder: Mario Rossi

2. **Azienda**
 - Label: Azienda
 - Placeholder: Ragione sociale

3. **Email**
 - Label: Email aziendale
 - Placeholder: nome@azienda.com

4. **Messaggio**
 - Label: La tua richiesta
 - Placeholder: Prodotto, quantità, mercato e punto di consegna di riferimento.

**Microcopy consenso GDPR (sotto al form, checkbox o testo):**
Inviando il modulo acconsenti al trattamento dei dati per essere ricontattato in merito alla tua richiesta, secondo la [Privacy Policy](/privacy). Non usiamo i tuoi dati per altro.

**Label bottone:** Invia richiesta

**Microcopy rassicurazione (sotto al bottone):**
Risposta entro 1–2 giorni lavorativi. Le richieste vengono lette dal team operativo.

**Dati di contatto (a fianco del form):**
- **Sede:** Via Aldo Sandulli 91, 00156 Roma (RM), Italia
- **Email:** info@fortunsrl.com
- **Telefono:** `[DA CONFERMARE: telefono pubblico]`

**Destinazione submit:** `/grazie`

---

## SEZIONE 8 — FOOTER

**Tagline:**
Trading internazionale di prodotti petroliferi raffinati. Da Roma ai principali hub del mondo.

**Dati aziendali:**
- **Ragione sociale:** Fortun S.r.l.
- **Sede legale:** Via Aldo Sandulli 91, 00156 Roma (RM), Italia
- **P.IVA / C.F.:** 16828791000
- **Codice EORI:** IT16828791000
- **PEC:** 16828791000@impresa.italia.it
- **REA:** RM-1678563
- **Capitale sociale:** € 10.000,00 (versato € 2.500,00)

**Email:** info@fortunsrl.com

**Toggle lingua:** IT / EN

**Link legali:**
- [Privacy Policy](/privacy)
- [Cookie Policy](/cookie)

**Riga di chiusura:**
© 2026 Fortun S.r.l.. Tutti i diritti riservati.

---

## PAGINA /grazie (NOINDEX)

**Titolo:**
Richiesta ricevuta.

**Messaggio:**
Grazie. La tua richiesta è arrivata al team operativo di Fortun.

**Tempi di risposta:**
Ti ricontattiamo entro 1–2 giorni lavorativi all'indirizzo che ci hai indicato. Se la richiesta è urgente, scrivici direttamente a info@fortunsrl.com.

**Link ritorno home:** Torna alla home → `/`

---

## PAGINA /privacy

**Titolo:**
Privacy Policy

**Intro:**
Informativa sul trattamento dei dati personali ai sensi del Regolamento (UE) 2016/679 (GDPR).

`[DA CONFERMARE: testo legale completo a cura del consulente legale / DPO]`

---

## PAGINA /cookie

**Titolo:**
Cookie Policy

**Intro:**
Informativa sull'utilizzo dei cookie e delle tecnologie di tracciamento su questo sito.

`[DA CONFERMARE: testo legale completo a cura del consulente legale / DPO]`

---

# 🇬🇧 ENGLISH — mirror (ramo `/en`)

## NAV

**Logo / Wordmark:** FORTUN

**Navigation items (internal anchors):**
- Positioning → `#posizionamento`
- Operating hubs → `#hub`
- Products → `#prodotti`
- Trading & supply → `#servizi`
- How we operate → `#come-operiamo`
- Contact → `#contatti`

**Language toggle:** IT / EN (persistent, right side of nav)

**Nav CTA:** Contact the team → `#contatti`

---

## SECTION 1 — HERO

**Eyebrow:** International trading · Refined petroleum products

**Headline:**
Trading and supply of refined petroleum products for markets in constant evolution.

**Sub (factual line):**
Based in Rome · operating across Rotterdam, Houston and Fujairah · diesel, jet fuel, LPG, fuel oil, crude oil.

**Primary CTA:** Request supply → `#contatti`

---

## SECTION 2 — POSITIONING

`#posizionamento`

**Eyebrow:** Who Fortun is

**Headline:**
An Italian trading company, with a network of suppliers and counterparties worldwide.

**Body:**
Fortun SRL is an Italian company based in Rome. We trade refined petroleum products: we buy, sell and coordinate supply between producers, refineries, distributors and industrial operators. We do not extract and we do not refine — we move product that is already on-spec, from the markets where it enters circulation to the markets where it is needed. Our work is knowing where supply sits, where demand sits, and handling everything in between.

**Anchor figures:**
- **2022** — year the company was founded
- **5** refined products handled
- 3 operating hubs across three time zones — ARA, US Gulf, Middle East

---

## SECTION 3 — OPERATING HUBS

`#hub`

**Eyebrow:** Where we operate

**Headline:**
Three hubs, at the main junctions of the refined product trade.

**Intro:**
Our operations rest on the three nodes that concentrate much of the world's refined product flows. Each hub gives us access to storage, counterparties and logistics in its area.

**Hub 1 — Rotterdam**
- **Macro-area:** ARA (Amsterdam–Rotterdam–Antwerp)
- **Role:** The European reference point for refined product storage and distribution. From here we coordinate supply into Northern Europe and the Atlantic basin.

**Hub 2 — Houston**
- **Macro-area:** US Gulf (Gulf of Mexico, USA)
- **Role:** Access to the refining capacity and export flows of the US Gulf. The hub for operations on the American and transatlantic market.

**Hub 3 — Fujairah**
- **Macro-area:** Middle East (United Arab Emirates)
- **Role:** A junction for bunkering and transit between the Persian Gulf, Asia and East Africa. From here we cover the flows moving east.

---

## SECTION 4 — PRODUCTS

`#prodotti`

**Eyebrow:** What we handle

**Headline:**
Five refined products, traded on-spec.

**Intro:**
We work with standardized products recognized on the international market. Every supply is defined by specification, quantity and delivery point.

**Product 1 — Diesel EN590**
Automotive gasoil compliant with the European EN590 standard. The reference fuel for road transport and industrial use.

**Product 2 — Jet Fuel A1**
Aviation kerosene (Jet A1). Turbine fuel for commercial air transport.

**Product 3 — LPG**
Liquefied petroleum gas. Used for heating, automotive fuel and industrial applications.

**Product 4 — Fuel Oil**
Fuel oil for industrial and marine use, including marine bunkering.

**Product 5 — Crude Oil**
Crude bound for refineries as process feedstock.

---

## SECTION 5 — TRADING & SUPPLY

`#servizi`

**Eyebrow:** What we do

**Headline:**
From sourcing to delivery, we manage the entire commercial operation.

**Intro:**
Our role does not end at putting two parties in touch. We follow the supply along the full chain, from finding the product to coordinating delivery.

**Service 1 — International commodities trading**
We buy and sell refined petroleum products on international markets, managing price, counterparties and contract terms.

**Service 2 — Global sourcing and procurement**
We identify product availability at refineries, producers and suppliers, assessing origin, specification and counterparty reliability.

**Service 3 — Supply coordination**
We manage the timing and the stages of supply between origin and destination, aligning volumes, logistics and delivery points.

**Service 4 — Trade facilitation**
We structure transactions between the parties, handling documentation, terms and conditions of the deal.

**Service 5 — International market operations**
We operate on the global refined product markets, reading availability, demand and flows between regions.

---

## SECTION 6 — HOW WE OPERATE

`#come-operiamo`

**Eyebrow:** Our method

**Headline:**
How we work, and why those who deal with us know what to expect.

**Intro:**
In a market where counterparty reliability counts as much as price, the way we operate is part of what we offer.

**Pillar 1 — Reliable operations**
We take on only supply we can see through to the end. We set clear terms before committing, and we hold to what was agreed on specification, volumes and timing.

**Pillar 2 — Global presence**
With hubs in Rotterdam, Houston and Fujairah, we cover the areas where most of the refined product trade concentrates. A presence across several time zones lets us operate without interruption.

**Pillar 3 — Compliance-oriented approach**
We work in line with the regulations governing the international trade in petroleum products. We vet counterparties and document operations along the full chain.

**Pillar 4 — Strategic relationships**
Commodity trading rests on the continuity of relationships. We build lasting ties with suppliers and counterparties, because a reliable channel is worth more than a single transaction.

---

## SECTION 7 — CONTACT

`#contatti`

**Eyebrow:** Let's talk supply

**Headline:**
Have a supply or purchase enquiry? Write to us.

**Microcopy (above the form):**
Tell us the product, quantity and target market. We'll reply with the next steps.

**FORM — fields:**

1. **Name**
 - Label: Full name
 - Placeholder: John Smith

2. **Company**
 - Label: Company
 - Placeholder: Registered company name

3. **Email**
 - Label: Business email
 - Placeholder: name@company.com

4. **Message**
 - Label: Your enquiry
 - Placeholder: Product, quantity, market and target delivery point.

**GDPR consent microcopy (below the form, checkbox or text):**
By submitting this form you consent to your data being processed so we can contact you regarding your enquiry, in accordance with the [Privacy Policy](/en/privacy). We don't use your data for anything else.

**Button label:** Send enquiry

**Reassurance microcopy (below the button):**
Reply within 1–2 business days. Enquiries are read by the operations team.

**Contact details (alongside the form):**
- **Office:** Via Aldo Sandulli 91, 00156 Rome (RM), Italy
- **Email:** info@fortunsrl.com
- **Phone:** `[DA CONFERMARE: telefono pubblico]`

**Submit destination:** `/en/thank-you`

---

## SECTION 8 — FOOTER

**Tagline:**
International trading of refined petroleum products. From Rome to the world's main hubs.

**Company details:**
- **Legal name:** Fortun S.r.l.
- **Registered office:** Via Aldo Sandulli 91, 00156 Rome (RM), Italy
- **VAT / Tax code:** 16828791000
- **EORI number:** IT16828791000
- **PEC:** 16828791000@impresa.italia.it
- **REA:** RM-1678563
- **Share capital:** € 10,000.00 (paid-in € 2,500.00)

**Email:** info@fortunsrl.com

**Language toggle:** IT / EN

**Legal links:**
- [Privacy Policy](/en/privacy)
- [Cookie Policy](/en/cookie-policy)

**Closing line:**
© 2026 Fortun S.r.l.. All rights reserved.

---

## /en/thank-you PAGE (NOINDEX)

**Title:**
Enquiry received.

**Message:**
Thank you. Your enquiry has reached Fortun's operations team.

**Response time:**
We'll get back to you within 1–2 business days at the address you gave us. If your enquiry is urgent, write to us directly at info@fortunsrl.com.

**Return-home link:** Back to home → `/en`

---

## /privacy PAGE

**Title:**
Privacy Policy

**Intro:**
Information on the processing of personal data pursuant to Regulation (EU) 2016/679 (GDPR).

`[DA CONFERMARE: testo legale completo a cura del consulente legale / DPO]`

---

## /cookie PAGE

**Title:**
Cookie Policy

**Intro:**
Information on the use of cookies and tracking technologies on this site.

`[DA CONFERMARE: testo legale completo a cura del consulente legale / DPO]`
